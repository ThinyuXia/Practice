...
string nu[10000]; 
int qq = 0;
int isLetter(char a) { //判断是否是字母请完成此函数
	if((a >= 'a' && a <= 'z') || (a >= 'A' && a <= 'Z'))
		return 1;
	else
		return 0;
...
int isDigit(char a)  { //判断是否是数字请完成此函数
	if(a >= '0' && a <= '9')
		return 1;
	else 
		return 0;
...
int alpha(int st){ //识别保留字和标识符，给此函数的的语句加上注释
	...
	for (int k = 0; k < 20; k++) { //遍历关键字字符串数组，每个keywords[k] 代表一个关键字
	if (strcmp(keywords[k].c_str(),wordbuf) == 0)  //如果关键字字符串数组中有关键字和wordbuf相等，flag = 1；
		flag = 1;
	}
	if (flag == 0){ //flag = 0 说明在没个keywords没有找到关键字
		int idx = -1; //寻找合法关键字或非法关键字起始下标
		for (int t = 0; t < pp; t++){ // pp 代表当前标识符个数，根据pp对id数组进行遍历
			if (strcmp(id[t].c_str(), wordbuf) == 0){ //利用idx确定标识符起始位置
				idx = t; //完成赋值
			}
		}
		if (idx != -1) printf(" (id,%d) ", idx);  //说明找到指定标识符，输出标识符位置
		else{
		...
		printf(" (");
		for (int i = 0; i < n; i++){ //n是wordbuf数组长度根据n输出wordbuf字符
...
int number(int st){ //识别整数
	char numbuf[20] = " "; int n = 0; //初始化长度20的字符数组，用于存放数字
...
int anotation(int st){ //处理除号和注释
	char tabuf[9999] = " "; // 该数组用与保存字符'/'后面的字符串
	int n = 0; //作为wordbuf数组的下标指针，指向wordbuf的最后一个字符
	st ++; //st自增，指向rz数组中'/'的下一个字符
	if (rz[st] == '/'){ //当前字符是'/'说明这段语句是注释
		printf(" (//,-)"); //输出字符串  (//,-)
		st++; // st自增
		while (rz[st] != 10){  //读到换行符终止
	...
	}else if (rz[st] == '*'){  // 当前字符是'*'说明这段语句是多行注释
		printf(" (/*,-) ");  //输出字符串  (/*,-) 
		st++; //st自增
		int stt = st + 1; 
		while (1){
...
int main(){
		int i = 0;
		FILE* fp;
		char name[10]; printf("请输入文件名:\n"); //读取文件路径
		scanf("%s", name);
		if ((fp = fopen(name, "r")) == NULL) { //如果存在指定文件则打开
		    printf("Open error!");
		    exit(0); 
		}
		char ch = fgetc(fp); 
	    ...
	    int j = 0;
		while (rz[j] != '\0')
			j = choice(j); cout << endl << " 程序中标示符如下 " << endl; 
