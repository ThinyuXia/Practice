...
//对栈进行初始化
	for (int i = 0; i <= num; i++)
		...
		while ( n <0 ) {
			...
			if (n == 14||n==16||n==17)
				        //请填写程序语句；
                        rightLength = production[n ].size() - 5;
				else if(n==1)
					    //请填写程序语句；
                        rightLength = production[n ].size() - 8;
					else
			...
			if (n == 10) { //当用 10.E→i 式归约时,开始建立栈
                ...
                --quaternion_stack.top; //请填写程序语句；
                ss2 = quaternion_stack.s[quaternion_stack.top];//请填写程序语句；
				...
				quaternion_stack.top = 0;
				syfout << now << " (=:," << quaternion_stack.s[quaternion_stack.top]<< ",-," << symbol_stack.st[l] << ")" << endl;
				++now;
			} else if (n == 13 || n == 14 || n == 15 || n == 16) //判断语句的四元式
				syfout << now << " if " << symbol[i - 2] << symbol[i - 1] << symbol[i]<< " goto " << start << endl;
//------------------------------------------------------
...
// showLex(state_stack,symbol_stack,i);
			GOTO(GetStateTop(state_stack), production[n][0], state_stack,symbol_stack);//此处用到了状态栈和符号栈的引用
